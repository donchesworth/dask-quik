...
# Install conda
ENV PATH="/opt/conda/envs/drp/bin:/opt/conda/bin:$PATH"
RUN yum -y install wget && \
    wget --quiet https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -O ~/miniconda.sh && \
    chmod +x ~/miniconda.sh &&  ~/miniconda.sh -b -p /opt/conda && conda update conda

# conda package installs
RUN conda create -n drp python=3.8 && \
    conda install --name drp -c rapidsai -c nvidia -c conda-forge -c defaults \
